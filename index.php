...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check_email = $conn->prepare("SELECT * FROM users WHERE LOWER(email) = LOWER(?)"); // case insensitive check
    ...
    if ($result->num_rows > 0) {
        ...
        if ($password !== $confirm_password) {
            ...
            // Insert the new user into the database (email_verified = 1 because no verification is needed now)
            $stmt = $conn->prepare("INSERT INTO users (firstname, lastname, email, password, email_verified) VALUES (?, ?, ?, ?, 1)");
